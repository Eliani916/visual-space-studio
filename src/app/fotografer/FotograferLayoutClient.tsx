...
export default function FotograferLayoutClient({ children, user }: FotograferLayoutClientProps) {
  ...
  const getPageTitle = () => {
    if (pathname.includes("/gallery/")) {
      return "Kelola Galeri Foto";
    }
    const activeItem = navigation.find((item) => pathname.startsWith(item.href));
    return activeItem ? activeItem.name : "Fotografer Panel";
  };
                ...
                <>
                  ...
                  <div className="absolute right-0 mt-2.5 w-60 rounded-2xl bg-white dark:bg-zinc-900 border border-slate-200/80 dark:border-zinc-800/80 shadow-xl py-2 z-50 animate-in fade-in slide-in-from-top-3 duration-200">
                    <div className="px-4 py-3 border-b border-slate-100 dark:border-zinc-800/60">
                      ...
                      <p className="text-sm font-bold text-slate-800 dark:text-zinc-100 truncate">{user?.name || "Fotografer"}</p>
                      <p className="text-xs font-semibold text-slate-400 dark:text-zinc-500 mt-2">Email</p>
                      <p className="text-xs text-slate-650 dark:text-zinc-350 truncate">{user?.email || "foto@example.com"}</p>
                      <p className="mt-2.5">
                    ...
                    <div className="p-1.5">
                      <button
                        ...
                        className="w-full flex items-center gap-2.5 px-3 py-2 text-xs font-bold text-red-650 hover:bg-red-50 dark:hover:bg-red-955/20 rounded-xl transition cursor-pointer border-0 w-full"
                      >
